...
:: 环境变量设置
set adb_path=%script_path%..\..\MPhone\MPhoneCore\src\third_party\adb\win\*.*
set jar_path=%script_path%..\..\MPhone\MPhoneCore\src\third_party\scrcpy-server
...
if /i %cpu_mode% == x86 (
    ...
    if exist %qt_msvc_path%\msvc2019\bin (
    ...
    if exist %qt_msvc_path%\msvc2019_64\bin (
...
:: 添加qt依赖包
windeployqt %publish_path%\MPhone.exe
