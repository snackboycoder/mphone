...
appdir_path="$appimage_output_path/MPhone.AppDir"
app_name="MPhone"
app_version=$(cat MPhone/appversion 2>/dev/null || echo "0.0.0")
...
if [ -f "$output_path/mphone-agent.apk" ]; then
    cp "$output_path/mphone-agent.apk" "$appdir_path/usr/bin/"
elif [ -f "$project_root/agent/android/build/mphone-agent.apk" ]; then
    cp "$project_root/agent/android/build/mphone-agent.apk" "$appdir_path/usr/bin/mphone-agent.apk"
fi
...
adb_source="$project_root/MPhone/MPhoneCore/src/third_party/adb/linux/adb"
server_source="$project_root/MPhone/MPhoneCore/src/third_party/scrcpy-server"
mkdir -p "$appdir_path/usr/lib/mphone"

if [ -f "$adb_source" ]; then
    cp "$adb_source" "$appdir_path/usr/lib/mphone/adb"
    chmod +x "$appdir_path/usr/lib/mphone/adb"
    # Create symlink for sndcpy.sh compatibility
    if [ ! -f "$appdir_path/usr/bin/adb" ]; then
        ln -sf "../lib/mphone/adb" "$appdir_path/usr/bin/adb"
    fi
fi

if [ -f "$server_source" ]; then
    cp "$server_source" "$appdir_path/usr/lib/mphone/scrcpy-server"
    chmod +x "$appdir_path/usr/lib/mphone/scrcpy-server"
fi
...
if [ -f "$project_root/MPhone/res/MPhone.png" ]; then
    icon_source="$project_root/MPhone/res/MPhone.png"
elif [ -f "$project_root/MPhone/res/image/tray/logo.png" ]; then
    icon_source="$project_root/MPhone/res/image/tray/logo.png"
elif [ -f "$project_root/MPhone/res/MPhone.ico" ]; then
    icon_source="$project_root/MPhone/res/MPhone.ico"
fi
...
<component type="desktop-application">
  ...
  <description>
    <p>MPhone supports displaying and controlling Android devices via USB or over network. It does NOT require root privileges.</p>
  </description>
  <provides>
    <binary>$app_name</binary>
  </provides>
  <launchable type="desktop-id">$app_name.desktop</launchable>
  <url type="homepage">https://github.com/mphone/MPhone</url>
...
export MPHONE_ADB_PATH="$HERE/usr/lib/mphone/adb"
export MPHONE_SERVER_PATH="$HERE/usr/lib/mphone/scrcpy-server"
export MPHONE_CONFIG_PATH="$HERE/usr/share/config"
exec "$HERE/usr/bin/MPhone" "$@"
APPRUN_EOF
else
    cat > "$appdir_path/AppRun" << 'APPRUN_EOF'
#!/bin/bash
...
export QML_SOURCES_PATHS="$project_root/MPhone"
