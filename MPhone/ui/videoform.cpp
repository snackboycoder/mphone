...
void VideoForm::updateShowSize(const QSize &newSize)
{
    if (m_frameSize != newSize) {
        ...
        if (m_embeddedMode) {
            ...
            ui->keepRatioWidget->setMinimumSize(0, 0);
            updateGeometry();
            return;
        }
...
void VideoForm::mousePressEvent(QMouseEvent *event)
{
    ...
    if (m_videoWidget->geometry().contains(event->pos())) {
        ...
        if (!m_videoWidget->size().isEmpty()) {
            emit mirrorMouseInput(m_serial, event->type(),
                                  QPointF(mappedPos.x() / m_videoWidget->width(), mappedPos.y() / m_videoWidget->height()),
                                  event->button(), event->buttons(), event->modifiers());
        }

        // debug relative input position
...
void VideoForm::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (m_dragPosition.isNull()) {
        ...
        if (!m_videoWidget->size().isEmpty()) {
            emit mirrorMouseInput(m_serial, event->type(),
                                  QPointF(local.x() / m_videoWidget->width(), local.y() / m_videoWidget->height()),
                                  event->button(), event->buttons(), event->modifiers());
        }
    } else {
...
void VideoForm::wheelEvent(QWheelEvent *event)
{
    ...
    if (m_videoWidget->geometry().contains(event->pos())) {
        ...
        if (!m_videoWidget->size().isEmpty()) {
            emit mirrorWheelInput(m_serial, QPointF(pos.x() / m_videoWidget->width(), pos.y() / m_videoWidget->height()),
...
void VideoForm::keyPressEvent(QKeyEvent *event)
{
    ...
    emit mirrorKeyInput(m_serial, event->type(), event->key(), event->modifiers(), event->text(), event->isAutoRepeat(), event->count());
}
...
{
    Q_UNUSED(event)
    if (!m_embeddedMode && !isFullScreen() && this->show_toolbar) {
...
void VideoForm::dropEvent(QDropEvent *event)
{
    ...
    for (const QUrl &url : urls) {
        ...
        if (!fileInfo.exists()) {
            QMessageBox::warning(this, "MPhone", tr("file does not exist"), QMessageBox::Ok);
            continue;
        }
