...
bool Server::readInfo(VideoSocket *videoSocket, QString &deviceName, QSize &size)
{
    ...
    while (videoSocket->bytesAvailable() < (DEVICE_NAME_FIELD_LENGTH + 12)) {
        videoSocket->waitForReadyRead(120);
        if (timer.elapsed() > 5000) {
            qInfo("readInfo timeout, available=%lld need=%d",
                  static_cast<long long>(videoSocket->bytesAvailable()),
                  DEVICE_NAME_FIELD_LENGTH + 12);
            return false;
        }
    }
    ...
    qInfo("readInfo ok: device=%s size=%dx%d",
...
void Server::onConnectTimer()
{
    ...
    QTcpSocket *controlSocket = m_params.control ? new QTcpSocket() : nullptr;

    videoSocket->connectToHost(QHostAddress::LocalHost, m_params.localPort);
    if (!videoSocket->waitForConnected(700)) {
        qWarning("video socket connect to server failed, try again");
        goto result;
    }

    if (m_params.control) {
        controlSocket->connectToHost(QHostAddress::LocalHost, m_params.localPort);
        if (!controlSocket->waitForConnected(700)) {
            qWarning("control socket connect to server failed, try again");
            goto result;
        }
    }
